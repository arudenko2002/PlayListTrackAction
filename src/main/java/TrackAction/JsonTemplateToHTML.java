...
public class JsonTemplateToHTML {
    ...
    public JsonTemplateToHTML() throws Exception {
        ...
        BufferedReader br = new BufferedReader(new FileReader("resources/playlist-add-drop2.html"));
        try {
    ...
    private int processHTML(int index, ArrayList<String> ar, String cmd) {
        ...
        for(int i=index;i<ar.size();i++) {
            if(ar.get(i).startsWith("<<<END OF PLAYLIST")
                    || ar.get(i).startsWith("<<<END OF TRACK")
                    || ar.get(i).startsWith("<<<END OF ARTIST")
                    ) {
                return i+1;
            }
            ...
            if(cmd.equals("artist") && !ar.get(i).startsWith("<<<")) {
                artistProlog  += ar.get(i)+"\n";
            }
            if(ar.get(i).startsWith("<<<TRACK")) {
            ...
            if(cmd.equals("track") && !ar.get(i).startsWith("<<<")) {
                trackProlog  += ar.get(i)+"\n";
            }
            if(ar.get(i).startsWith("<<<PLAYLIST")) {
            ...
            if(cmd.equals("playlist") && !ar.get(i).startsWith("<<<")) {
